"""
🔬 TESTE CONTROLADO - Cópia EXATA do baseline test_html_ssr_50urls.py
ZERO mudanças para confirmar se baseline ainda funciona @ 1.006s
"""
...
async def testar_50_urls():
    ...
    print("=" * 100)
    print("🔬 TESTE CONTROLE - Cópia EXATA do baseline (test_html_ssr_50urls.py)")
    print("=" * 100)
    ...
    print(f"📝 ZERO mudanças - testar se ainda dá 1.006s/produto")
    print("=" * 100)
    print()
    ...
    async with httpx.AsyncClient() as client:
        ...
        print("=" * 100)
        print("📊 RESULTADOS FINAIS - BASELINE EXATO")
        print("=" * 100)
        ...
        print()
        print(f"🎯 ESPERADO: 1.006s/produto (50.32s total)")
        print(f"🎯 OBTIDO:   {tempo_medio:.3f}s/produto ({tempo_total:.2f}s total)")
        
        diff_percent = ((tempo_medio - 1.006) / 1.006 * 100)
        if abs(diff_percent) < 5:
            print(f"✅ DENTRO DA MARGEM: {diff_percent:+.1f}%")
        else:
            print(f"⚠️  FORA DA MARGEM: {diff_percent:+.1f}%")
        print("=" * 100)
        
        # Salvar resultados
        arquivo_resultado = 'resultados_baseline_control.json'
